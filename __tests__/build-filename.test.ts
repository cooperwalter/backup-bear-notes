...
import buildFilename from '../src/build-filename.js';
...
describe('buildFilename', () => {
  ...
  describe('filename length truncation', () => {
    ...
    it('should handle multi-byte unicode characters when truncating', () => {
      const unicodeTitle = '\u{1F43B}'.repeat(100);
    ...
    it('should not split multi-byte characters when truncating', () => {
      const unicodeTitle = 'a'.repeat(249) + '\u{1F43B}';
      const result = buildFilename(unicodeTitle);
      expect(Buffer.byteLength(result, 'utf8')).toBeLessThanOrEqual(255);
      expect(result.endsWith('.md')).toBe(true);
  ...
  describe('error handling', () => {
    it('should throw TypeError for null input', () => {
      expect(() => buildFilename(null as unknown as string)).toThrow(TypeError);
    });

    it('should throw TypeError for undefined input', () => {
      expect(() => buildFilename(undefined as unknown as string)).toThrow(TypeError);
    });

    it('should throw TypeError for number input', () => {
      expect(() => buildFilename(123 as unknown as string)).toThrow(TypeError);
    });

    it('should throw TypeError for object input', () => {
      expect(() => buildFilename({} as unknown as string)).toThrow(TypeError);
    });
